...
if($cls=='All Classes')
{
	...
	{ 
		...
		$total=0;
		//echo $row['cls'];
		$sel1=mysql_query("select * from stdnt_reg where cls='$cls' && strm='".$arr_strmm['sno']."' && `continoue_discontinoue_status`='0' && `hstl`='Yes'");
...
if(!empty($amt_dues) ||  !empty($amt_old_hostel))
{
	echo $tr;
	$count_due++;
}
...
<h4><b>Total no. of students whose Hostel Fee is due :-  <?php echo $count_due; ?></b></h4>
</table>
...
$sr_no=0;
$count_due=0;
$sel1=mysql_query("select * from stdnt_reg where cls='$cls'  and `continoue_discontinoue_status`='0' and `rte`='No' && `hstl`='Yes' $condition");
while($arr1=mysql_fetch_array($sel1))
{ 
	$sel_tc=mysql_query("select * from `tc_serial_no` where `schlr_no_id`='".$arr1['id']."' && `status`='1'");
	$tc=mysql_num_rows($sel_tc);
