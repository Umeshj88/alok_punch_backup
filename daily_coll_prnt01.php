...
<link href="assets/css/print_page.css">
            ...
            $sel1=mysql_query("select * from annl_fee where dat='$i' order by `recpt_no` ASC");
            ...
            <tr>
            <td  align="center"></td><td align="center">General Fees</td><td align="right"><?php echo $amt1; ?></td>
            </tr>
            <?php
            }
            if($k>0 || $m>0 || $p>0)
            {
				if(!empty($r_ad1) && !empty($r_non1) && !empty($r22))
				{
            		$minimum_value=min($r_ad1,$r_non1,$r22);
				}
				...
				$maximum_value=max($r_ad,$r_non,$r2);
             $mon_fee+=($dep1+$dep_ad);
			 if($mnth_no==0)
			{
